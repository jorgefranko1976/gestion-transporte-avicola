...
export function useAuthState() {
  ...
  useEffect(() => {
    ...
    const forceTimeout = setTimeout(() => {
      if (mounted) {
        console.log('Forzando finalización de carga desde useAuthState (50ms timeout)');
        setIsLoading(false);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, currentSession) => {
        ...
        if (currentSession?.user) {
          try {
            // Usar directamente los metadatos del usuario para una respuesta inmediata
            ...
            // Intentar obtener perfil completo en segundo plano
            fetchUserProfile(currentSession.user.id)
              .then(profile => {
                if (profile && mounted) {
                  setUser(prevUser => ({
                    ...
                    role: (profile.role || prevUser?.role || 'driver') as User['role'],
                    profile: {
              ...
              .catch(error => console.error('Error al obtener perfil completo:', error))
              .finally(() => {
                if (mounted) {
                  setIsLoading(false);
                  setInitializationComplete(true);
