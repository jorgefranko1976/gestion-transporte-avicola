...
const DateRangeReportsTab = () => {
  ...
  const [dispatches, setDispatches] = useState<DateRangeReport[]>([]);
  const [filteredDispatches, setFilteredDispatches] = useState<DateRangeReport[]>([]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Filtrar empresas únicas y eliminar valores nulos o vacíos
        const uniqueOrigins = [...new Set(originsData
          .map(item => item.loading_company)
        ...
        // Filtrar destinos únicos y eliminar valores nulos o vacíos
        const uniqueDestinations = [...new Set(destinationsData
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const formattedDispatches = await Promise.all(dispatchesData.map(async (item) => {
        ...
        return {
          id: item.id,
          orderId: item.order_id || 'Sin ID',
          date: new Date(item.created_at),
          origin: item.loading_company || 'No especificado',
          destination: item.destination || 'No especificado',
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      const filtered = dispatches.filter(d => 
        ...
        (d.origin && d.origin.toLowerCase().includes(lowercaseSearch)) ||
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">Reportes por Rango de Fechas</h2>
        ...
        <div className="space-y-2">
          ...
          <Select value={origin} onValueChange={setOrigin}>
            ...
            <SelectContent>
              ...
              {origins.map(o => (
                <SelectItem key={o} value={o || "origen_desconocido"}>{o || "Origen desconocido"}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select value={destination} onValueChange={setDestination}>
            ...
            <SelectContent>
              ...
              {destinations.map(d => (
                <SelectItem key={d} value={d || "destino_desconocido"}>{d || "Destino desconocido"}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
              ...
              <tbody className="divide-y">
                {filteredDispatches.map((dispatch) => (
                  <tr key={dispatch.id} className="hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3">{dispatch.driverName}</td>
                    <td className="px-4 py-3">
...
export default DateRangeReportsTab;
